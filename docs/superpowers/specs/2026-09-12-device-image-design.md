...
- **Corrected 2026-09-20, from a card.** What follows reasoned from a package listing and got the live value wrong. systemd also ships `/usr/lib/sysctl.d/50-default.conf`, which sets `kernel.sysrq = 0x01b6`; that, not the compiled-in `0x1f6`, is what a panel runs with. The difference is `0x40`: `e`, `f` and `i` (SIGTERM, the OOM killer, SIGKILL to everything but init) are **not** available over this UART. `b`, `s`, `u` and console log level are. See `docs/hardware-checks.md`, "Read off the card". The original paragraph is kept as written:
