...
What the panel needs from a network is a short list: join Wi-Fi, DHCP, DNS, NTP, and **outbound** TLS to AWS IoT (MQTT 8883) and HTTPS. It accepts **no inbound connection, ever**. It has no login, no console prompt and no unlocked account. Everything beyond that list is surface that a rink's public Wi-Fi, a hotel network or a stranger's living room can reach, for no benefit to the panel.
...
1. **`dtoverlay=disable-bt` in `/boot/firmware/config.txt`**, appended under an explicit `[all]` so a pi-gen bump that adds a trailing board-specific section cannot quietly scope it to one board. Read from the overlay's own source, it sets `&bt` to `status = "disabled"`, disables `&uart1` (the mini UART), enables `&uart0` (the PL011) on GPIO 14/15, and repoints `/aliases serial0` at `/soc/serial@7e201000`. Four consequences, each checked:
   ...
   - **Both target boards are covered.** The overlay README: "On Pis prior to Pi 5 this restores UART0/ttyAMA0 over GPIOs 14 & 15" — which is the Pi 4 and the Zero 2 W. (`disable-bt-pi5` exists for the other case; this image does not target it.)
   - **The console survives.** `cmdline.txt` says `console=serial0,115200`, and the firmware substitutes `serial0` from `/aliases`, which the overlay repoints at the PL011 — so the line resolves to `ttyAMA0` instead of `ttyS0`. It is more capable than what is there today, not less: the real boots show the firmware passing `8250.nr_uarts=0`, so the `ttyS0` that `console=serial0` currently resolves to does not exist. Either way the console is not this panel's diagnosis surface; the persistent journal (§9.2) is.
...
**Autoremove.** The sub-stage runs none, for the reason `03-no-remote-access` gives: `export-image/02-set-sources` runs one afterwards regardless, so adding one here would change nothing. What that later run can take is leaf shared libraries — `libavahi-core7` and `libdaemon0` (nothing but `avahi-daemon` depends on them), `libfido2-1`, `libwrap0` and `libwtmpdb0` (nothing but OpenSSH). `libavahi-common3` is **not** among them: `libcups2t64` depends on it. Neither is `libbluetooth3`.
...
- the eight masks asserted individually, each as a symlink **resolving** to `/dev/null` inside the image;
...
The gate reads a filesystem. It cannot see a port. Nothing here proves the hardened image still boots, still joins Wi-Fi and still enrolls — the three things this change could break and the two costs the branch was told to respect. The regression check on **H5** and **H8**, and the new LAN scan that looks for an mDNS name, an open TCP port and a Bluetooth advertisement, are in `docs/hardware-checks.md` under the 2026-09-19 note.
