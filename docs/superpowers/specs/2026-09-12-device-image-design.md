...
- **The journal is persistent** (added 2026-09-18). journald's *resolved* `Storage` is `persistent` and `/var/log/journal` exists as a real directory. The resolved value, not the presence of §9.2's drop-in: journald sorts every drop-in from `/etc`, `/run`, `/usr/local/lib` and `/usr/lib` by filename across all of them at once and the last one to set an option wins, so a file named `99-anything.conf` could put `Storage=volatile` back and a rule that only looked for our own file would wave it through. The gate replays that ordering (ties on the same filename going to `/etc`) over `journald.conf` and both drop-in directories. This is the assertion that a panel which fails to start can still be diagnosed at all.
