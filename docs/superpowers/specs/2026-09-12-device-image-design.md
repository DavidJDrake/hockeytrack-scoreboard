...
- **The hardened image has a serial console on GPIO 14/15, which the image before it did not.** `dtoverlay=disable-bt` makes the PL011 the primary UART, `enable_uart` then defaults to 1, and the `console=serial0,115200` already in `cmdline.txt` becomes live (§9.13). It is **kept deliberately**: it is the diagnosis path a panel with no login, no getty on `tty1` and a black screen has never had, and it is the nearest thing to the failure painter this section keeps asking for. What it is, plainly: a **physical-access surface**. Someone holding the panel with a 3.3 V USB-serial adapter reads the boot log, and the journal too if `systemd.journald.forward_to_console=1` is added to the card. They could already read the same journal by taking the card. The **login prompt** systemd would put on that console is masked (`serial-getty@.service`, the template), because every account is locked and it would serve nobody. Two things about it are **unmeasured**: the kernel's compiled-in `sysrq` default, since nothing on this image sets `kernel.sysrq`; and what synchronous `printk` to a 115200 UART costs a boot that is already dark for up to 85 s. H9 measures the second, and §9.13 records the one-line way to back the decision out.
...
**The console is kept.** It is the diagnosis path this project has needed on every failed boot. v0.1.0, v0.1.1 and v0.1.2 were each diagnosed by powering the panel down and reading the card; §9.12 has been asking for an on-panel failure painter ever since, and that is still a follow-up. A 3.3 V USB-serial adapter on pins 8 and 10 now reads the boot log live, and with `systemd.journald.forward_to_console=1` added to `cmdline.txt` by hand — the procedure `docs/hardware-checks.md` already documents — it reads the journal live too, with no card removal. It costs nothing to anyone who does not attach a cable, and it cannot be reached over a network.
...
- **Whatever the kernel's sysrq default is.** Nothing on this image sets `kernel.sysrq` (`raspberrypi-sys-mods`' `98-rpi.conf` sets only `printk`, `min_free_kbytes` and `ping_group_range`), so the compiled-in kernel default applies and it was **not measured**. Anyone at that UART can already pull the power and the card, so this is not an escalation — but it is not nothing, and it is unverified.
- **Boot time.** `printk` to a 115200 UART is synchronous, so a boot that previously wrote to `tty1` alone now also serializes every kernel line out the UART, cable attached or not. On a panel that is already dark for up to 85 s (§9.12) that matters, and it is **unmeasured**. `docs/hardware-checks.md` H9 part 1 measures first paint against v0.1.3 for exactly this reason.
