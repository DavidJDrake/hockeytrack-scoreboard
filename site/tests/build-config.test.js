...
// hash, redeploys code that did not change, and pages for nothing. Once people
// learn to ignore those pages, a real one is ignored too. This keeps the build
// reproducible.
...
test("every Lambda build is reproducible across commits and checkouts", () => {
  // A commented-out line would satisfy the flags without building anything, so
  // only recipe lines count.
  const builds = makefile.split("\n").filter((line) => !/^\s*#/.test(line) && /\$\(GO\) build\b/.test(line));
