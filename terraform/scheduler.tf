...
resource "aws_iam_role_policy" "scheduler" {
  ...
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [{ Effect = "Allow", Action = "lambda:InvokeFunction", Resource = [aws_lambda_function.today.arn, aws_lambda_function.imagecheck.arn, aws_lambda_function.summary.arn, aws_lambda_function.director.arn] }]
  })
}
